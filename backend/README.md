...
```bash
mvp_app_protegge_e_trasforma$ sam build --use-container
```
...
```bash
mvp_app_protegge_e_trasforma$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
mvp_app_protegge_e_trasforma$ sam local start-api
mvp_app_protegge_e_trasforma$ curl http://localhost:3000/
```
...
```bash
mvp_app_protegge_e_trasforma$ sam logs -n HelloWorldFunction --stack-name mvp_app_protegge_e_trasforma --tail
```
...
```bash
mvp_app_protegge_e_trasforma$ pip install -r tests/requirements.txt --user
# unit test
mvp_app_protegge_e_trasforma$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
mvp_app_protegge_e_trasforma$ AWS_SAM_STACK_NAME="mvp_app_protegge_e_trasforma" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "mvp_app_protegge_e_trasforma"
```
