...
void main() {
  group('HomeDashboardWidget Widget Test', () {
    ...
    testWidgets('Deve mostrare esattamente 4 DashboardButtonWidget', (WidgetTester tester) async {
      await pumpDashboard(tester);
      expect(find.byType(DashboardButtonWidget), findsNWidgets(4));
    });

    testWidgets('Deve mostrare il pulsante "Supporto Chat"', (WidgetTester tester) async {
      await pumpDashboard(tester);
      expect(find.text('Supporto Chat'), findsOneWidget);
    });

    testWidgets('Deve mostrare il pulsante "Contatti Fidati"', (WidgetTester tester) async {
      await pumpDashboard(tester);
      expect(find.text('Contatti Fidati'), findsOneWidget);
    });

    testWidgets('Deve mostrare il pulsante "Il mio Diario"', (WidgetTester tester) async {
      await pumpDashboard(tester);
