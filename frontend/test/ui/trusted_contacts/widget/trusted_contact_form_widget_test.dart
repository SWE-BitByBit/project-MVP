...
void main() {
  ...
  group('TrustedContactFormWidget - Validazione & Rendering', () {
    testWidgets('mostra messaggi di errore se si tenta di salvare con campi vuoti', (tester) async {
    ...
    testWidgets('pre-popola i campi se viene passato un initialContact e adatta i testi', (tester) async {
      ...
      await tester.pumpWidget(createWidgetUnderTest(initialContact: existingContact));
  ...
  group('TrustedContactFormWidget - Creazione Contatto', () {
    testWidgets('chiama createContact con i dati inseriti e invoca onDismiss su successo', (tester) async {
      await tester.pumpWidget(createWidgetUnderTest());

      // Compiliamo il form
      await tester.enterText(find.widgetWithText(TextFormField, 'Nome e Cognome'), 'Luigi Bianchi');
      await tester.enterText(find.widgetWithText(TextFormField, 'Numero di Cellulare'), '3339876543');
      await tester.enterText(find.widgetWithText(TextFormField, 'Indirizzo Email'), 'luigi@email.com');

      await tester.tap(find.text('Salva contatto'));
      await tester.pumpAndSettle();

      // Verifica l'oggetto catturato
      final captured = verify(() => mockCreateCommand.runAsync(captureAny())).captured;
    ...
    testWidgets('NON invoca onDismiss se createContact emette errori', (tester) async {
      await tester.pumpWidget(createWidgetUnderTest());

      await tester.enterText(find.widgetWithText(TextFormField, 'Nome e Cognome'), 'Test Error');
      await tester.enterText(find.widgetWithText(TextFormField, 'Numero di Cellulare'), '123');
      await tester.enterText(find.widgetWithText(TextFormField, 'Indirizzo Email'), 'a@b.c');
  ...
  group('TrustedContactFormWidget - Modifica Contatto', () {
    testWidgets('chiama updateContact mantenendo l\'ID originale', (tester) async {
      final existingContact = TrustedContact(
        id: 'ABC-123',
        name: 'Vecchio Nome',
      ...
      await tester.enterText(find.widgetWithText(TextFormField, 'Nome e Cognome'), 'Nuovo Nome');

      await tester.tap(find.text('Aggiorna contatto'));
      await tester.pumpAndSettle();

      final captured = verify(() => mockUpdateCommand.runAsync(captureAny())).captured;
      final contactPassed = captured.first as TrustedContact;
  ...
  group('TrustedContactFormWidget - Stato Caricamento', () {
    testWidgets('mostra il CircularProgressIndicator sul bottone durante l\'esecuzione (modalità Creazione)', (tester) async {
      createRunningNotifier.value = true;

      await tester.pumpWidget(createWidgetUnderTest());
      await tester.pump(); // Usiamo pump e non pumpAndSettle per via dell'animazione di caricamento
