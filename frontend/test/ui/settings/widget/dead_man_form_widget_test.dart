...
void main() {
  ...
  group('DeadManFormWidget - Rendering Stati Iniziali', () {
    testWidgets(
      'Mostra CircularProgressIndicator quando in caricamento e senza draft',
      (tester) async {
    ...
    testWidgets(
      'Mostra ErrorIndicator se il caricamento fallisce a draft vuota',
      (tester) async {
        when(() => mockVm.draftSettings).thenReturn(null);
  ...
  group('DeadManFormWidget - Rendering Form (Dati caricati)', () {
    testWidgets('Popola correttamente tutti i campi dalla draftSettings', (
      tester,
    ) async {
      await tester.pumpWidget(createWidgetUnderTest());
      await tester.pumpAndSettle();
    ...
    testWidgets(
      'Il bottone Salva è disabilitato se hasUnsavedChanges è falso',
      (tester) async {
        when(() => mockVm.hasUnsavedChanges).thenReturn(false);
        ...
        final saveButton = tester.widget<FilledButton>(
          find.byKey(Key("dms_save_button")),
        );
    ...
    testWidgets(
      'Mostra CircularProgressIndicator nel bottone durante il salvataggio',
      (tester) async {
        ...
        await tester.pumpWidget(createWidgetUnderTest());
        await tester
            .pump(); // Usiamo pump invece di pumpAndSettle per via dell'animazione infinita

        // Dentro il Row del bottone Salva
        final circularProgressFinder = find.descendant(
          of: find.byKey(Key("dms_save_button")),
  ...
  group('DeadManFormWidget - Interazioni Utente', () {
    testWidgets('Tap sullo Switch chiama toggleActiveStatus nel VM', (
      tester,
    ) async {
    ...
    testWidgets('Digitare nei TextField chiama updateMessage nel VM', (
      tester,
    ) async {
      ...
      await tester.enterText(
        find.widgetWithText(TextFormField, 'Emergenza'),
        'Nuovo Oggetto',
      );

      // Il widget richiama updateMessage con il nuovo testo e il vecchio body
      verify(
        () => mockVm.updateMessage(
          'Nuovo Oggetto',
          'Aiuto, controllate se sto bene.',
        ),
      ).called(1);
    });

    testWidgets('Tap su Salva chiama runAsync e mostra SnackBar di successo', (
      tester,
    ) async {
      when(() => mockVm.hasUnsavedChanges).thenReturn(true);
      ...
      // Tap bottone Salva
      await tester.tap(find.byKey(Key("dms_save_button")));
